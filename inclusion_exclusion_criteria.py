...
def get_vote_on_paper(data, paper_id, inclusion_criteria, title = False):
    ...
    for i in range(0, len(inclusion_criteria)):
        skip = 0
        try:
            if isinstance(data.loc[paper_id, inclusion_criteria[i]], np.bool_) is True or isinstance(data.loc[paper_id, inclusion_criteria[i]], bool) is True:
                ...
                # If keywords from group energy services, pass title as relevant
                    ...
                    displayed_paper_info = paper_author_year_title_abstract(data, paper_id, title, displayed_paper_info)
            else:
                # No value for inclusion criteria stored - print information to receive a vote
                displayed_paper_info = paper_author_year_title_abstract(data, paper_id, title, displayed_paper_info)
        except:
...
def save_and_quit(data):
    ...
    if all([key in data.columns for key in inclusion_criteria_title_and_abstract]):
        ...
        print(f"Intermediate number of otherwise relevant papers: {data[inclusion_criteria_title_and_abstract[6]].sum()}")
    sys.exit()
...
def evaluate_title_and_abstract():
    ...
    for i in data.index:
        ...
        try:
            ...
            value = None
            print(f"The title inclusion criteria of paper {data.loc[i,TITLE]} is None, as DataFrame returns {data.loc[i, inclusion_criteria_title[0]]}.")
